...
"""
OBSERVATIONS: The sm model is a smaller model than md, and does not utilise word vectors, and so we simply get a
similarity method via the tokenization of each element; and we receive a UserWarning for this. As such, we note that the
md model can pick up more contextual similarities in language due to its larger size, and hence when running the sm
model, we generally observe lower scores across the board.
"""
